...
function MainApp() {
  ...
  // Proper Login Gate: Full-screen 50/50 split authentication view
  if (!token || !user) {
    return (
      <div className="h-screen w-full flex flex-col lg:flex-row overflow-hidden font-sans selection:bg-emerald-500 selection:text-white animate-login-enter">
        {/* Left Half: Brand & Architecture Banner (50vw, 100vh fitted) */}
        <div className="w-full lg:w-1/2 h-full bg-[#0a0f24] flex flex-col justify-between relative overflow-hidden border-b lg:border-b-0 lg:border-r border-slate-800/80">
          <FigmaHeroBanner />
        </div>

        {/* Right Half: Clean, Professional Sign-In Form (50vw, 100vh fitted) */}
        <div className="w-full lg:w-1/2 h-full bg-white flex flex-col justify-between px-6 sm:px-10 lg:px-12 py-5 overflow-y-auto">
          <div className="w-full max-w-md mx-auto my-auto">
            <FigmaSignInForm />
          </div>

          <div className="text-center py-1 text-[10px] text-slate-400 font-medium tracking-tight">
            Microfinancial Management System (MMS) • Enterprise 256-Bit Cryptographic Security • DOLE & BIR TRAIN Law Compliant
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="h-screen bg-[#f8fafc] flex font-sans antialiased text-slate-900 overflow-hidden animate-app-enter">
      {/* 1:1 Figma Dark Sidebar with Dynamic User Card and Logout Button */}
      <Sidebar 
      ...
      {/* Main Content Area — scrolls independently while sidebar stays sticky */}
        ...
        <main className="flex-1 p-6 sm:p-8 max-w-7xl w-full mx-auto">
          <div key={activeModule} className="animate-module-fade">
            {!hasModuleAccess(user?.role, activeModule) ? (
              <div className="bg-white rounded-3xl border border-red-200/80 shadow-sm p-8 sm:p-12 text-center max-w-xl mx-auto my-12 space-y-4">
                ...
                <div className="pt-2">
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs shadow-md transition-all inline-flex items-center space-x-2 cursor-pointer"
                  >
